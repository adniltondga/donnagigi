...
interface LoginClientProps {
  /** Vem do server (force-dynamic) — já sabemos em render 1, sem flicker. */
  registrationOpen: boolean
}

export default function LoginClient({ registrationOpen }: LoginClientProps) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center text-muted-foreground">Carregando...</div>}>
      <LoginInner registrationOpen={registrationOpen} />
    </Suspense>
  )
}

function LoginInner({ registrationOpen }: LoginClientProps) {
  const router = useRouter()
  const params = useSearchParams()
  // Lazy-init pelo ?register=1 — evita render 1 com tab "Entrar" e
  // render 2 trocando pra "Criar conta" (causa do flicker reportado).
